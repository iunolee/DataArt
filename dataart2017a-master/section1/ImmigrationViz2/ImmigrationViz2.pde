...
void setup() {
  ...
  //Turn Canada red
  countryMap.get("Mexico").c = #FF0000;
}
...
void sortByForeignBorn() {
  for (Country c : allCountries) {
    c.tpos.x = map(c.foreignBorn, 5000, 50000000, 50, width - 50);
  }
}
...
void loadData(String url) {
  ...
  for (int i = 0; i <data.getRowCount(); i++) {
    ...
    // Set the size
    c.s = map(sqrt(c.foreignBorn), sqrt(5000), sqrt(50000000), 10, 100);


    c.errorBounds[0] = map(sqrt(c.foreignBorn - error), sqrt(1000), sqrt(50000000), 10, 100);
    c.errorBounds[1] = map(sqrt(c.foreignBorn + error), sqrt(1000), sqrt(50000000), 10, 100);
